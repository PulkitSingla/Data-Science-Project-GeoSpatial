...
def get_data_columns(df, category, frequency="monthly"):
    if frequency == "monthly":
        if category.lower() == "county":
            ...
            del_cols = ["month_date_yyyymm", "cbsa_code",
...
def join_attributes(gdf, df, category):
    ...
    if category == "county":
        ...
        new_gdf = gdf.merge(df, left_on="STUSPS",
                            right_on="STUSPS", how="outer")
    elif category == "national":
        if "geo_country" in df.columns.values.tolist():
            df["country"] = None
            df.loc[0, "country"] = "United States"
        new_gdf = gdf.merge(df, left_on="NAME",
                            right_on="country", how="outer")
    elif category == "metro":
        new_gdf = gdf.merge(df, left_on="CBSAFP",
                            right_on="cbsa_code", how="outer")
    elif category == "zip":
        new_gdf = gdf.merge(df, left_on="GEOID10",
                            right_on="postal_code", how="outer")
    return new_gdf
...
def get_weeks(df):
    seq = list(set(df[~df["week_end_date"].isnull()]
                   ["week_end_date"].tolist()))
    weeks = [
        datetime.date(int(d.split("/")[2]),
                      int(d.split("/")[0]), int(d.split("/")[1]))
        for d in seq
    ]
    weeks.sort()
    return weeks
...
def app():
    ...
    data_cols = get_data_columns(
        inventory_df, scale.lower(), frequency.lower())
    ...
    with row2_col2:
        n_colors = st.slider("Number of colors",
